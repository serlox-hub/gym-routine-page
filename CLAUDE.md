...
- ❌ Reordering a list with a drag wired by hand in the screen. Drag lives in `ui/SortableList.jsx` (web, `@dnd-kit`) and `ui/DraggableList.jsx` (native, RNGH + Reanimated), with the same API (`items`/`renderItem`/`onReorder(from, to, activeId, offsetX)`/`disabled`, optional `collapseForDrag`/`resolveDrop`/`getDragPreview`; native also takes the `scrollRef`). `onReorder` fires even with `from === to` (a horizontal shift in place can change membership): callers that only reorder discard it. Drag **always starts on a handle** (`ui/DragHandle.jsx`), never on the whole row: that keeps the card's press, the header menu and finger scrolling intact. The row gets an opaque `dragHandleProps` and imports nothing from the gesture library. **The position menu stays**: it is the only screen-reader path. The arithmetic (where it lands, how far a neighbour moves, auto-scroll speed) is in `lib/dragReorder.js`; its functions carry `'worklet'` because native calls them from the UI thread (`babel-preset-expo` already configures the worklets plugin; the Reanimated 3 one is NOT added, see the comment in `babel.config.js`). A **day's exercise list** is a FLAT list of rows (one header per superset plus one row per exercise, `buildExerciseRows`), not nested lists, and each row paints its slice of the purple card (`segment`: the header the top edge, each member the sides, the last one the bottom) with padding and gap INSIDE its own height, because the arithmetic sums row heights. Where each row can land lives ONLY in `lib/exerciseOrder.js`, never in the component: a header moves its whole run and never changes membership; an exercise can land in any slot and **its membership comes from the slot** (`resolveMembershipDrop`: right below a header or between two members joins that run; right after a run's last member is an edge decided by the horizontal offset against `design.supersetIndent`: an individual, or a member of another run, joins when dragged at least that far right; a member at its own run's edge stays unless dragged that far left; anything else is individual). The list gets the rules as `collapseForDrag`/`resolveDrop` (bound with `allowMembershipChange`) and `getDragPreview` (`getMembershipDropPreview`), and the drop resolves through `applyRowDrop` with the same offset: the preview is exactly what is saved. The preview is a LOOK, not a sideways shift: the dragged row is painted as a slice of the purple card or flush (the card's own side padding is the indent), keeping its resting vertical space so no row below jumps; `supersetIndent` is only the threshold. No drag creates a superset. Which rows get a handle is `canDragExerciseRow`: a member of a run of more than one always (even as the block's only unit), a header or an individual only with another unit to pass, and the member of a one-member superset never (it leaves through the action sheet). The **position menu keeps the old restriction**: the header goes through `moveSuperset` and an exercise through `getExerciseReorderScope` + `moveExercise`, so a member only moves within its run and the menu never changes membership. A handle blocks its row's swipe **on press** (`onPressStart`/`onPressEnd` of `DragHandle` → `blockedRef` of `useSwipeToDelete`), not when the drag activates: the swipe is classified earlier (web on the first `pointermove`, native inside `onMoveShouldSetPanResponder`)
