...
- **Decisiones no obvias y "cómo se implementó X" a nivel feature** → entrada en `docs/DECISIONS.md` (fecha, qué, **por qué**, cómo, alternativas descartadas, gotchas). Ej.: por qué una subcarpeta concreta en un bucket, por qué una dependencia nativa concreta, por qué una versión de esquema. **El listón: ¿una sesión futura leyendo el código podría plausiblemente deshacer esto?** Si el porqué se ve en el código, no hay entrada; si es un hecho sobre cómo funciona algo y no una elección entre opciones, va al README. **La mayoría de los cambios no producen ninguna entrada** — hinchar el log es lo que hace que deje de leerse, y eso cuesta más que la entrada que falta.
