...
CREATE OR REPLACE FUNCTION "public"."duplicate_routine_day"("p_day_id" integer, "p_new_name" "text") RETURNS "public"."routine_days"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
...
ALTER FUNCTION "public"."duplicate_routine_day"("p_day_id" integer, "p_new_name" "text") OWNER TO "postgres";
...
COMMENT ON FUNCTION "public"."reorder_routine_exercises"("exercise_orders" "jsonb") IS 'Reorders the exercises of ONE routine day in batch. Takes a JSONB array of {id, sort_order, superset_group?}: superset_group is written only when the key is present (null = leave the superset).';
