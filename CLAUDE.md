...
**Fuente de verdad del estado ACTUAL**: `apps/web/supabase/schema.sql` (dump schema-only generado, commiteado). Consulta ESE archivo para saber qué tablas/columnas/constraints/RPCs existen HOY. Las migraciones (`apps/web/supabase/migrations/`) son el ledger append-only para *aplicar* cambios, malas para *consultar* el estado actual (p. ej. `routine_blocks` se creó y luego se eliminó en la 031). Regenerar con `npm run db:schema` (requiere Docker). ⚠️ Ese script empieza por `supabase db reset`, que reconstruye la BD **local** desde cero: es justo eso lo que garantiza `schema.sql == migraciones`, así que **no lo saltes dumpeando a secas** (sin reset el dump refleja tu BD local tal cual, migración nueva sin aplicar incluida, y el guard A7 no lo detecta: solo mira que `schema.sql` esté en el diff). El precio del reset lo pagas **solo si tu `.env` apunta a la Supabase local**: se lleva por delante el usuario de test de los e2e y hay que recrearlo a mano (`config.toml` siembra `./seed.sql`, que no existe, y `e2e/auth.setup.js` solo hace login, no signup). Con el `.env` apuntando al proyecto remoto — el default del repo — el reset no les afecta. El checklist de "cuando se modifique el modelo de datos" vive en `docs/routine-io.md`. Rationale y gotchas en `docs/DECISIONS.md` (issue #19). El diagrama de abajo es solo orientación de alto nivel.
