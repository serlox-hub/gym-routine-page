  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} position="bottom" maxWidth="max-w-lg" className="max-h-[80vh] flex flex-col" noBorder>
      <div className="p-4 shrink-0" style={{ borderBottom: `1px solid ${colors.border}` }}>
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            {routineDayId && (
              <div className="shrink-0 flex rounded-full p-0.5" style={{ backgroundColor: colors.bgTertiary }}>
                <button onClick={() => setScope(SCOPE.DAY)} className="px-2 py-0.5 rounded-full text-xs font-medium transition-colors"
                  style={{ backgroundColor: scope === SCOPE.DAY ? 'rgba(63, 185, 80, 0.2)' : 'transparent', color: scope === SCOPE.DAY ? colors.success : colors.textSecondary }}>
                  Rutina
                </button>
                <button onClick={() => setScope(SCOPE.GLOBAL)} className="px-2 py-0.5 rounded-full text-xs font-medium transition-colors"
                  style={{ backgroundColor: scope === SCOPE.GLOBAL ? 'rgba(139, 148, 158, 0.2)' : 'transparent', color: scope === SCOPE.GLOBAL ? colors.textPrimary : colors.textSecondary }}>
                  Global
                </button>
              </div>
            )}
        ...
        <div className="flex" style={{ backgroundColor: colors.bgTertiary, borderRadius: '8px', padding: '4px' }}>
