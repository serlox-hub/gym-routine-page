...
- [x] **SETUP-01**: Root `package.json` configures npm workspaces with `apps/web`, `apps/gym-native`, and `packages/core`
- [x] **SETUP-02**: `packages/core/package.json` has `"main": "./src/index.js"` pointing at source, no build step
...
- [ ] **UTIL-01**: 16 pure utility files live in `packages/core/src/lib/`
- [ ] **UTIL-02**: 18 test files co-located with utils, vitest passes from root
- [ ] **UTIL-03**: Barrel `index.js` re-exports all shared modules
- [ ] **UTIL-04**: `supabase.js`, `styles.js`, `videoStorage.js` excluded from core (platform-specific)

### API Layer

- [ ] **API-01**: 7 API files live in `packages/core/src/api/`
- [ ] **API-02**: API functions receive Supabase client as parameter (no direct import of supabase.js)
- [ ] **API-03**: Web hooks updated to import API from `@gym/core`
- [ ] **API-04**: RN hooks refactored from inline Supabase queries to shared API functions

### Stores

- [ ] **STOR-01**: `createWorkoutStore(storage)` factory in `packages/core/src/stores/`
- [ ] **STOR-02**: `createAuthStore(supabase, storage)` factory in `packages/core/src/stores/`
...
- [ ] **HOOK-01**: Shared TanStack Query hooks in `packages/core/src/hooks/`
...
- [ ] **DX-01**: `QUERY_KEYS` and domain constants shared in `packages/core` (text diff reconciled)
- [ ] **DX-02**: `queryClient.js` config shared in `packages/core`
...
| Build step for packages/core | Internal-only package, both bundlers resolve source directly |
