...
- **El detalle de rutina NO tiene modo edición** (issue #85): la pantalla siempre pinta las afordancias de edición (asa de arrastre en los días cuando hay 2 o más, swipe para borrar en los ejercicios, menú de acciones al pulsar la fila) y el nombre/descripción de la rutina se editan en un modal (`RoutineEditForm` dentro de `RoutineHeader`) con Guardar/Cancelar explícitos. No hay autoguardado: el que había perdía la última pulsación si la pantalla se desmontaba dentro de su debounce. Como no hay deshacer, **la confirmación es la única guarda de cada borrado**: ningún borrado de esta pantalla puede quedarse sin su `ConfirmModal`, y el mensaje debe nombrar lo que se borra. `/routine/:id/edit` sobrevive solo como redirección (`pages/RoutineEditRedirect.jsx`).
