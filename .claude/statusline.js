...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Git branch (best-effort, skip optional locks)
    let branch = '';
    try {
      const { execSync } = require('child_process');
      const raw = execSync(
        `git -C ${JSON.stringify(dir)} symbolic-ref --short HEAD 2>/dev/null`,
        { stdio: ['ignore', 'pipe', 'ignore'], timeout: 1000, env: { ...process.env, GIT_OPTIONAL_LOCKS: '0' } }
      ).toString().trim();
      if (raw) branch = ` \x1b[36m${raw}\x1b[0m`;
    } catch (e) {
