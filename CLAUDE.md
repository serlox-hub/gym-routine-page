...
- **El detalle de rutina NO tiene modo edición** (issue #85): la pantalla siempre pinta las afordancias de edición (asa de arrastre en los días cuando hay 2 o más, swipe para borrar en ejercicios y días (en el día se escucha la cabecera sin el asa y se desliza la tarjeta entera), menú de acciones al pulsar la fila) y el nombre/descripción de la rutina se editan en un modal (`RoutineEditForm` dentro de `RoutineHeader`) con Guardar/Cancelar explícitos. No hay autoguardado: el que había perdía la última pulsación si la pantalla se desmontaba dentro de su debounce. Como no hay deshacer, **la confirmación es la única guarda de cada borrado**: ningún borrado de esta pantalla puede quedarse sin su `ConfirmModal`, y el mensaje debe nombrar lo que se borra. `/routine/:id/edit` sobrevive solo como redirección (`pages/RoutineEditRedirect.jsx`).
- **Overlay a pantalla completa (web)**: siempre vía `Modal` (`components/ui/Modal.jsx`), que bloquea el scroll del fondo fijando el body (`lib/bodyScrollLock.js`: `position:fixed` + `top:-scrollY`, con contador porque los modales se apilan; `overflow:hidden` no basta en iOS Safari). Un `fixed inset-0` a pelo (como el `RestTimer` a pantalla completa, que no encaja en `Modal` por su layout propio) deja que la página de detrás siga scrolleando salvo que llame a `lockBodyScroll`/`unlockBodyScroll` directamente — y reimplementar el bloqueo aparte en vez de reusar ese módulo pisa el contador de `Modal`.
...
El em dash y el punto y coma los enforza `i18n/copyStyle.test.js`; el resto (tono, `Etiqueta: detalle`, relleno) es juicio de revisión.
...
**Fuente de verdad del estado ACTUAL**: `apps/web/supabase/schema.sql` (dump schema-only generado, commiteado). Consulta ESE archivo para saber qué tablas/columnas/constraints/RPCs existen HOY. Las migraciones (`apps/web/supabase/migrations/`) son el ledger append-only para *aplicar* cambios, malas para *consultar* el estado actual (p. ej. `routine_blocks` se creó y luego se eliminó en la 031). Regenerar con `npm run db:schema` **desde `apps/web`** (requiere Docker, y las `GYM_SUPABASE_*` en el `.env`/`.env.local` de `apps/web` — sin ellas la CLI muere con un `ProjectConfigParseError` que no nombra ni variable ni archivo, ver issue #63 al final de este documento; desde la raíz la CLI se inventa otro proyecto y crea un `supabase/` vacío ahí). ⚠️ Ese script empieza por `supabase db reset`, que reconstruye la BD **local** desde cero: es justo eso lo que garantiza `schema.sql == migraciones`, así que **no lo saltes dumpeando a secas** (sin reset el dump refleja tu BD local tal cual, migración nueva sin aplicar incluida, y la revisión de `/gate` no lo detecta: solo mira que `schema.sql` esté en el diff — así se perdió la RPC `duplicate_routine_day` del snapshot, ver `docs/DECISIONS.md`). Desde el issue #59 el CI **sí** compara el contenido: reconstruye la BD desde las migraciones y falla si el dump no coincide con el `schema.sql` commiteado, así que un snapshot desfasado ya no llega a `main` — pero te enteras en el CI, no antes de pushear. El gate `schemaDrift` de `/gate` (`npm run db:check-drift`) adelanta ese mismo diff a antes del commit, pero **se salta** si el stack local no está levantado o si la BD no tiene aplicadas exactamente las migraciones del árbol, así que sigue habiendo que comprobar que `schema.sql` viaja en el diff junto a la migración. El reset no cuesta nada: `supabase/seed.sql` recrea el usuario `e2e@local.test` en cada uno y los e2e ya resetean la BD ellos mismos, así que la local es desechable por diseño. El checklist de "cuando se modifique el modelo de datos" vive en `docs/routine-io.md`. Rationale y gotchas en `docs/DECISIONS.md` (issue #19). El diagrama de abajo es solo orientación de alto nivel.
...
- **El ritmo va siempre por kilómetro** (`PACE_DISTANCE_UNIT`), nunca compuesto con la unidad de distancia. Porqué en el JSDoc de `PACE_DISTANCE_UNIT` (`lib/measurementFields.js`).
...
Estos estándares ya están detallados en las secciones de abajo. El objetivo es cumplirlos desde la primera pasada, no que los cace una revisión posterior (`/gate` es la verificación final: no debería encontrar nada nuevo).
...
- **Decisiones no obvias y "cómo se implementó X" a nivel feature** → entrada en `docs/DECISIONS.md` (fecha, qué, **por qué**, alternativas descartadas, gotchas). Ej.: por qué una subcarpeta concreta en un bucket, por qué una dependencia nativa concreta, por qué una versión de esquema. **El listón: ¿una sesión futura leyendo el código podría plausiblemente deshacer esto?** Si el porqué se ve en el código, no hay entrada; si es un hecho sobre cómo funciona algo y no una elección entre opciones, va al README. **La mayoría de los cambios no producen ninguna entrada** — hinchar el log es lo que hace que deje de leerse, y eso cuesta más que la entrada que falta.
- **Podar `DECISIONS.md` es parte de tocarlo**: borra las entradas cuyo código ya no existe o cuya decisión se revirtió. Git guarda el historial; este fichero guarda solo lo que sigue siendo carga. Poda cuando toques el código que una entrada describe — nunca tires una entrada viva para hacer sitio.
- **"Porqué" local no evidente** (valor mágico, workaround, orden que importa) → comentario inline con el *por qué*, no el *qué*.
- **Env vars / config nuevas** → `.env.example` de ambas apps + descripción. Que estén declaradas lo enforza `apps/web/src/test/envExample.test.js`; la descripción no.
...
- ❌ Reordering a list with a drag wired by hand in the screen. Drag lives in `ui/SortableList.jsx` (web, `@dnd-kit`) and `ui/DraggableList.jsx` (native, RNGH + Reanimated), with the same API (`items`/`renderItem`/`onReorder(from, to, activeId)`/`disabled`, optional `collapseForDrag`/`resolveDrop`/`getDragPreview`/`withDropSlot`; native also takes the `scrollRef`). Drag **always starts on a handle** (`ui/DragHandle.jsx`), never on the whole row: that keeps the card's press, the header menu and finger scrolling intact. The row gets an opaque `dragHandleProps` and imports nothing from the gesture library. **The position menu stays**: it is the only screen-reader path. The arithmetic (where it lands, how far a neighbour moves, auto-scroll speed) is in `lib/dragReorder.js`; its functions carry `'worklet'` because native calls them from the UI thread (`babel-preset-expo` already configures the worklets plugin; the Reanimated 3 one is NOT added, see the comment in `babel.config.js`). A **day's exercise list** is a FLAT list of rows (one header and one footer per superset plus one row per exercise, `buildExerciseRows`), not nested lists, and each row paints its slice of the purple card (by `kind`: the header the top edge, each member the sides, the footer the bottom) with padding and gap INSIDE its own height, because the arithmetic sums row heights. Where each row can land lives ONLY in `lib/exerciseOrder.js`, never in the component: a header moves its whole run and never changes membership; an exercise can land in any slot and **its membership comes from the slot** (`resolveMembershipDrop`: with a header or a member of a run right above, it joins that run, including right above its footer, which is how it lands last; below a footer or anywhere else it is individual). Only the vertical position decides: the footer row is what tells "last in the superset" from "first after it", which are the same slot without it. The owner rejected a horizontal gesture for that. The list gets the rules as `collapseForDrag`/`resolveDrop` (bound with `allowMembershipChange`) and `getDragPreview` (`getMembershipDropPreview`), and the drop resolves through `applyRowDrop` with the same index: the preview is exactly what is saved. The preview is painted by the GAP, not by the row in flight: with `withDropSlot` the row follows the pointer flush (web through a `DragOverlay`) and the list paints `renderItem` with `isDropSlot` in the gap, at the row's height, as a slice of the purple card or empty. That is what keeps the card's sides continuous while dragging; a row in flight never lines up with them. The exercise list also passes `animateShift={false}`: rows jump instead of sliding, because each row slides at its own moment (only the one crossed moves) and mid-slide the card would show holes. No drag creates a superset. Which rows get a handle is `canDragExerciseRow`: a member of a run of more than one always (even as the block's only unit), a header or an individual only with another unit to pass, and the member of a one-member superset never (it leaves through the action sheet). The **position menu keeps the old restriction**: the header goes through `moveSuperset` and an exercise through `getExerciseReorderScope` + `moveExercise`, so a member only moves within its run and the menu never changes membership. A handle blocks its row's swipe **on press** (`onPressStart`/`onPressEnd` of `DragHandle` → `blockedRef` of `useSwipeToDelete`), not when the drag activates: the swipe is classified earlier (web on the first `pointermove`, native inside `onMoveShouldSetPanResponder`)
- ❌ Writing `superset_group` on an existing routine exercise from the list without placing the row in the same write. A superset is a run of CONSECUTIVE rows (`groupExercisesBySupersetId`), so a group written in place renders the superset as two purple cards with the same label. From the list, membership changes only through `setRoutineExerciseSupersetGroup` or `reorderRoutineExercises` (items carrying `supersetGroup`; the RPC writes `superset_group` only when the key is present, and rejects a payload spanning two days), both placing through `placeInSuperset`. `moveRoutineExerciseToDay` may null it: leaving the day cannot split anything. Known bugs that still write it unplaced: the edit form's superset select (`apps/web/src/pages/RoutineDetail.jsx`, `apps/gym-native/src/components/Routine/ExerciseConfigForm.jsx`), adding into a superset (`addExerciseToDay`, which appends at the end of the day), session → routine propagation (`updateSessionExerciseFields` in `api/sessionExercisesApi.js`), and the superset selectors spanning both blocks (`getExistingSupersetIds(allExercises)` in both `DayCard.jsx`)
- ❌ Reimplementar por plataforma las reglas de un gesto de fila. Cuándo un movimiento cuenta como swipe (dominancia 2:1 sobre el eje vertical, distancia medida solo en `dx`) y cuánto se mueve la fila viven en `lib/swipeGesture.js` (`shouldClaimSwipe`/`clampSwipeOffset`), y `blocked` entra en la DECISIÓN de reclamar, no después, para que el futuro arrastre-para-reordenar pueda impedirlo. Lo que sí se duplica a propósito es recordar en qué quedó el gesto: native tiene que clasificarlo dentro de `onMoveShouldSetPanResponder`, un predicado síncrono que no puede consultar estado compartido. Esa parte vive UNA vez por plataforma en `hooks/useSwipeToDelete.js` (web y native): una fila nueva con swipe la usa, no la copia. Ver `docs/DECISIONS.md` (issue #78)
...
| Cuerpo desplegado de un día y aplicación de un orden a su caché | `routineDayLayout.js` | `getRoutineDayLayout()`, `applyExerciseOrderToBlocks()`, `placeInSupersetForDay()` |
...
**Los e2e (Playwright) solo corren contra la Supabase LOCAL**, aquí y en CI (issue #55): `npm run test:e2e -w apps/web` empieza por `supabase db reset`, o sea que necesita Docker y el stack levantado (`npx supabase start` desde `apps/web`) y **reconstruye la BD local entera**. `scripts/assertLocalSupabase.js` (hook `pretest:e2e`, y también importado por `playwright.config.js`) aborta si `VITE_SUPABASE_URL` no es `127.0.0.1`/`localhost`: los specs escriben datos de verdad y nadie los limpia después. Sin el reset la suite empieza a fallar sola a la tercera ejecución seguida (los datos se acumulan y las rutinas creadas rompen locators de `createRoutine.spec.js`). ⚠️ **`test:e2e:ui` NO resetea**, a propósito: en el UI Mode relanzas a mano dentro de una ventana que sigue viva, así que un reset solo limpiaría la primera pasada, y encima se llevaría el estado del fallo que estás investigando. Si iterando empieza a fallar solo, lanza `npx supabase db reset` a mano. La configuración local va en un `.env.local` (no versionado), que gana sobre `.env` por la precedencia de Vite — replicada en `scripts/loadEnv.js` para que el guard y los setups vean lo mismo que el dev server. Mientras exista, todo lo que pasa por Vite apunta a la local: `dev`, `build`, `preview` y los tests unitarios (los deploys no: Vercel construye desde git y allí el archivo no existe). Valores exactos en `apps/web/.env.example`; rationale en `docs/DECISIONS.md`.

⚠️ **El stack local es POR WORKTREE, no compartido** (issue #63): `supabase/config.toml` interpola `GYM_SUPABASE_PROJECT_ID` y los 4 puertos activos desde el `.env`/`.env.local` de `apps/web`, que la CLI lee igual que Vite. **Son obligatorias**: un `env()` de puerto sin definir no cae a un default, revienta con `ProjectConfigParseError` y la CLI no arranca — copia `.env.example` antes del primer `supabase start`. Cada worktree lleva su propio bloque (convenio: sumar N*10 a cada puerto) porque `db reset` reconstruye la BD entera: con un id compartido, resetear desde una lane se llevaba por delante el trabajo de las otras.
