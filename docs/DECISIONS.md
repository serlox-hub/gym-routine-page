...
## 2026-07 · Editar inicio y fin de una sesión desde el historial
- **Cada extremo se edita y se guarda por separado**, con su resolución única: `resolveSessionEnd` (fin, acotado a [inicio, ahora]) y `resolveSessionStart` (inicio, acotado por `getSessionStartBounds`). La duración nunca se teclea: siempre `fin - inicio`. Descartado un único campo "mover de día" que desplazara los dos a la vez: arregla la sesión apuntada el día equivocado, pero no la que empezó con el cronómetro lanzado antes de tiempo. Coste aceptado: mover primero el inicio deja una duración de ~25h hasta que se mueve el fin.
- **La cota inferior del inicio existe por la columna**: `duration_minutes` es `smallint` y sin tope el UPDATE muere con un error de rango.
- **Entrada inválida en el inicio es un no-op** (devuelve el inicio actual), no un colapso contra el otro extremo como hace el fin: aquí el valor irrecuperable es el propio inicio.
- **El inicio se escribe con `rescheduleSession`, no con `updateSessionMetadata`**: `started_at` es la clave cronológica de los PRs, así que hay que resincronizar `exercise_session_stats.session_date` (reusando `recalculateSessionStats`, que ya lo deriva de la fila) y, solo si la sesión se movió hacia adelante, recalcular además la posición cronológica que dejó libre. Secuencial y sin transacción, como `reassignSessionGym`; idempotente, porque el paso 2 re-deriva de la sesión.
- **Si la resincronización falla, la mutación NO falla** (`statsSynced: false`, mismo trato que `useUpsertCompletedSet`): cuando llega ese paso la sesión ya está movida, así que propagar el error se salta `onSuccess` y deja la caché pintando la fecha vieja sobre una fila que ya cambió. Se prefiere una desnormalización que repara la siguiente edición a una pantalla que miente.
- **El historial SIGUE a la sesión que cambia de día** en vez de cerrar el detalle (`useSelectedDaySessions`, compartido por las dos pantallas): mover inicio y fin son dos pasos, y cerrar entre medias deja al usuario sin el sitio desde el que dar el segundo y con la duración de ~25h puesta. Solo se cierra si la sesión sale del mes cargado, que es cuando ya no está en la lista.
- ⚠️ **PRs empatados en el mismo `session_date`:** las CTEs de `recalculate_exercise_prs` ordenan solo por `session_date` y su marco es `ROWS` (no `RANGE`), así que dos sesiones en el mismo instante SÍ se ven entre sí, pero en el orden arbitrario que elija el planner. Como la comparación es estricta (`>`), de dos empatadas con el mismo valor solo marca PR la que quede primera, y cuál es puede cambiar entre recálculos idénticos. Antes era inalcanzable (`started_at` siempre `now()`); elegir el minuto a mano lo hace posible. Desempatar exige migración (añadir `id` al `ORDER BY` de cada CTE).
...
- **El final de una superserie lo marca una fila de pie, no un desplazamiento horizontal** (`buildExerciseRows` emite `supersetFooter`, que pinta el borde de abajo de la tarjeta). Sin pie, "último de la superserie" y "primero detrás de ella" son el mismo hueco en una lista vertical. Encima del pie entra, debajo sale, también con la superserie cerrando el bloque. Se implementó primero con desplazamiento horizontal ("profundidad proyectada" de las listas en árbol) y el dueño lo rechazó al probarlo: no se descubría, porque la vista previa no mueve la fila a los lados, y con la superserie al final no había forma de sacar un miembro hacia abajo. No hay umbral que ajustar: el tramo "encima del pie" sale de la geometría de las filas. **Las dos apps eligen destino por el centro de fila más cercano** (`closestCenter` en web, `getDropIndex` en native), así que el pie se cruza pasado ~(alto de tarjeta + alto del pie)/4. Native usaba "cruzar la mitad del vecino", y con un pie de ~9px bastaban 5px de temblor para entrar o salir.
