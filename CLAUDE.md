...
- ❌ Reordenar una lista con un arrastre cableado a mano en la pantalla. El arrastre vive en `ui/SortableList.jsx` (web, `@dnd-kit`) y `ui/DraggableList.jsx` (native, RNGH + Reanimated), con la misma API (`items`/`renderItem`/`onReorder(from,to)`/`disabled`; native además recibe el `scrollRef`). El arrastre **siempre arranca en un asa** (`ui/DragHandle.jsx`), nunca en la fila entera: es lo que deja intactas la pulsación de la tarjeta, el menú de dentro de la cabecera y el scroll con el dedo. La fila recibe `dragHandleProps` opaco y no importa nada de la librería de gestos. **El menú de posiciones se queda**: es el único camino con lector de pantalla. La aritmética (dónde cae, cuánto se aparta el vecino, velocidad de auto-scroll) está en `lib/dragReorder.js`; sus funciones llevan `'worklet'` porque native las llama desde el hilo de UI (`babel-preset-expo` ya configura el plugin de worklets; el de Reanimated 3 NO se añade, ver el comentario en `babel.config.js`). La **lista de ejercicios de un día** es una lista PLANA de filas (una cabecera por superserie más una fila por ejercicio, `buildExerciseRows`), no listas anidadas, y cada fila pinta su trozo de tarjeta morada (`segment`: la cabecera el borde de arriba, cada miembro los laterales, el último el de abajo) con el relleno y el hueco DENTRO de su propio alto, porque la aritmética suma altos de fila. Las reglas de dónde puede caer cada cosa (una superserie viaja entera, un miembro no sale de su tirada, un individual no entra en una) viven SOLO en `lib/exerciseOrder.js`: la lista las recibe como `collapseForDrag`/`resolveDrop` y al soltar se resuelve con `applyRowDrop`, nunca en el componente. El **menú de posiciones pasa por las mismas reglas**, no por índices planos: la cabecera con `moveSuperset`, y cada ejercicio con `getExerciseReorderScope` (qué posiciones puede ocupar) + `moveExercise`. Ofrecerle la lista plana del bloque dejaría al camino accesible partiendo superseries que el dedo no puede partir. Y un asa bloquea el swipe de su fila **al tocarla** (`onPressStart`/`onPressEnd` de `DragHandle` → `blockedRef` de `useSwipeToDelete`), no cuando el arrastre activa: el swipe se clasifica antes (web en el primer `pointermove`, native dentro de `onMoveShouldSetPanResponder`)
...
| Orden de los ejercicios de un bloque (unidades, filas, arrastre, menú) | `exerciseOrder.js` | `getBlockUnits()`, `moveExercise()`, `moveSuperset()`, `getExerciseReorderScope()`, `buildExerciseRows()`, `collapseForDrag()`, `resolveRowDrop()`, `rowDropToMove()`, `applyRowDrop()` |
| Cuerpo desplegado de un día y aplicación de un orden a su caché | `routineDayLayout.js` | `getRoutineDayLayout()`, `applyExerciseOrderToBlocks()` |
